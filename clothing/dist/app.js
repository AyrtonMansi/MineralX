...
function home(){
 ...
 <section class="closing-note"><p class="eyebrow">MINERAL / 01</p><h2>Less noise. More possibility.</h2><a class="text-link" href="${link()}">Explore the collection ${arrow}</a></section>`;
}
...
function route({focus=true}={}){
 ...
 main.innerHTML=path==='/'?home():path==='/collection'?collection(params):path.startsWith('/product/')?product(path.slice(9)):path==='/world'?world():path==='/fits'?fitPage():path.startsWith('/information/')?information(path.slice(13)):notFound();
...
dialog.addEventListener('close',()=>{document.body.classList.remove('modal-open');if(overlayTrigger?.isConnected)overlayTrigger.focus({preventScroll:true});});
...
function menu(){openOverlay('Explore X','menu-panel',`<nav class="mobile-nav" aria-label="Mobile navigation"><a href="#/collection?wearer=Men" data-action="navigate">Men ${arrow}</a><a href="#/collection?wearer=Women" data-action="navigate">Women ${arrow}</a><a href="#/collection" data-action="navigate">The collection ${arrow}</a>${categories.map(c=>`<a class="mobile-category" href="${link({category:c.name})}" data-action="navigate">${c.name}</a>`).join('')}<a href="#/world" data-action="navigate">The world of X ${arrow}</a><a href="#/fits" data-action="navigate">Find your fit ${arrow}</a></nav>`);}
document.addEventListener('click',async event=>{
 const el=event.target.closest('[data-action]');if(!el)return;
 const action=el.dataset.action;
 if(action==='close'||action==='navigate')closeOverlay();
...
window.addEventListener('storage',event=>{if(event.key===storageKey || event.key===null){try{saved=validateEdit(JSON.parse(event.newValue||'[]'));}catch{saved=[];}document.querySelector('#edit-count').textContent=saved.length;if(dialog.open&&dialog.classList.contains('edit-panel'))refreshEdit();}});
