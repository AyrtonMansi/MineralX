...
import {setup,ids} from './helpers';import {DevelopmentEngine} from '../../lib/ops/development-engine';
test('a version-six device archive upgrades atomically without replacing existing IDs, field data or campaign names',async()=>{
 ...
 try{
  ...
  assert.equal((await db.query<{v:number}>('select max(version) v from mx_ops.schema_version')).rows[0].v,7);
