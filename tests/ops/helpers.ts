...
export async function setup(extra=true){
 ...
 for(const name of ['202609060001_gic.sql','202609060002_run_timing.sql','20260907010000_operations_core.sql','20260907011000_operations_gold.sql',...(extra?['20260907012000_operations_geology.sql','20260907013000_operations_reads.sql','20260907014000_operations_hardening.sql']:[])]) {
...
export async function asUser(db:PGlite,user:string,sql:string,args:unknown[]=[],aal='aal2',role='authenticated'){
 await db.exec('begin');try{await db.query("select set_config('request.jwt.claim.sub',$1,true),set_config('request.jwt.claim.aal',$2,true)",[user,aal]);await db.exec('set local role '+role);const r=await db.query(sql,args);await db.exec('commit');return r.rows as Record<string,any>[];}catch(e){await db.exec('rollback');throw e;}
}
