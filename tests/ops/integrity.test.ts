...
test('operators may stage product results but cannot approve their own result',async()=>{
 ...
 await assert.rejects(command(db,ids.reviewer,'assay.stage',crypto.randomUUID(),{...p,unit:'g/kg'},1),/units do not agree/);
...
test('a document revision cannot supersede a document from another operation',async()=>{
 ...
 await assert.rejects(command(db,ids.operator,'document.publish',crypto.randomUUID(),{title:'Wrong scope',category:'procedure',file_id:source,supersedes:previous}),/workspace/);
 }finally{await db.close();}
});
