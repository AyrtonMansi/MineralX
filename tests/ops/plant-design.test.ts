...
const base=plantSchema.parse({
 ...
 equipment:[
  {id:'A',name:'Hammer crusher',x:2,y:2,w:3,h:2,group:'feed',basis:'test',note:'',symbol:[{points:[[2,2],[5,2],[5,4],[2,4],[2,2]],fill:true}],label:['Crusher',3.5,3],geographic:[[143,-19],[143.00002857,-19],[143.00002857,-18.99998198],[143,-18.99998198],[143,-19]]},
  {id:'B',name:'Russell J3 jig',x:14,y:8,w:3,h:3,group:'wet',basis:'test',note:'',symbol:[{points:[[14,8],[17,8],[17,11],[14,11],[14,8]],fill:true}],label:['Jig',15.5,9.5],geographic:[[143.00013333,-18.99992793],[143.0001619,-18.99992793],[143.0001619,-18.9999009],[143.00013333,-18.9999009],[143.00013333,-18.99992793]]},
 ],
...
test('concept equipment can be added with a stable semantic identity',()=>{
 ...
 assert.equal(compact.equipment.find(e=>e.id==='C')?.renderModel.archetype,'knudsen_bowl');
});

test('expert equipment archetypes are deterministic and remain explicitly inferred',()=>{
 const crusher=equipmentRenderProfile(base.equipment[0]),jig=equipmentRenderProfile(base.equipment[1]);
 assert.equal(crusher.archetype,'hammer_crusher');
 assert.equal(jig.archetype,'jig');
 assert.equal(jig.modelStatus,'inferred');
 assert.equal(jig.dimensions.cell_count,2);
});

test('ChatGPT can parametrically configure equipment without claiming unsupported authority',()=>{
 const result=applyPlantDesignOperations(base,[{type:'configure_equipment',equipmentId:'B',engineering:{archetype:'jig',modelStatus:'specified',overallHeightM:2.45,rotationDeg:12,dimensions:{cell_width_m:.62,deck_height_m:1.35},specification:[{key:'screen opening',value:'3',unit:'mm',basis:'operator-confirmed design basis'}]}}]);
 const equipment=result.model.equipment.find(e=>e.id==='B')!;
 assert.equal(equipment.engineering?.model_status,'specified');
 assert.equal(equipmentRenderProfile(equipment).heightM,2.45);
 assert.equal(equipmentRenderProfile(equipment).rotationDeg,12);
 assert.equal(equipmentRenderProfile(equipment).dimensions.cell_width_m,.62);
 assert.equal(result.validation.ok,true);
 assert.equal(result.validation.issues.some(issue=>issue.code==='equipment_geometry_inferred'),false);
 assert.notEqual(plantModelFingerprint(result.model),plantModelFingerprint(base));
});

test('inferred design changes remain visibly marked for later verification',()=>{
 const result=applyPlantDesignOperations(base,[{type:'configure_equipment',equipmentId:'A',engineering:{overallHeightM:3.4}}]);
 assert.equal(result.validation.ok,true);
 assert.ok(result.validation.issues.some(issue=>issue.code==='equipment_geometry_inferred'&&issue.targetId==='A'));
});
