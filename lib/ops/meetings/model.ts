...
export function extractCandidates(text:string){
  ...
  const start=lines.findIndex(l=>/^(?:next steps?|action items?|actions?(?: and next steps)?|follow[- ]?ups?)\s*:?\s*$/i.test(l.trim()));
  if(start<0)return [];
  ...
  for(let i=start+1;i<lines.length;i++){
    const line=lines[i],trim=line.trim();if(/^(?:summary|discussion|notes|decisions?)\s*:?\s*$/i.test(trim)){flush();break;}
