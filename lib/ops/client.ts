...
import {OPS_FILE_MAX_BYTES,OpsError,type Command} from './contracts';
import {developmentSession,DEVELOPMENT_ACTOR} from './development-policy';
...
export async function api<T=any>(path:string,data?:unknown,options:ApiOptions={}):Promise<T>{
 if(developmentSession()){const {developmentApi}=await import('./development');return developmentApi(path,data);}
 if(data && typeof data==='object' && (data as any).expectedActorId===DEVELOPMENT_ACTOR)throw new OpsError('forbidden','Development records cannot be submitted to production.');
 const timeoutMs=options.timeoutMs??60000;
 if(!Number.isSafeInteger(timeoutMs)||timeoutMs<1000||timeoutMs>120000)throw new OpsError('validation','The request timeout is outside the supported range.');
...
export async function uploadEvidence(scopeId:string,family:string,file:File,identity?:{id:string;requestId:string}){
 if(developmentSession()){const {developmentUpload}=await import('./development');return developmentUpload(scopeId,family,file,identity);}
 if(!file.size||file.size>OPS_FILE_MAX_BYTES)throw new Error('Choose a non-empty source file up to 50 MiB.');const bytes=await file.arrayBuffer();if(!bytes.byteLength||bytes.byteLength>OPS_FILE_MAX_BYTES)throw new Error('Choose a non-empty source file up to 50 MiB.');
 ...
 const ext=file.name.split('.').pop()?.toLowerCase();const media_type=ext==='kml'?'application/vnd.google-earth.kml+xml':ext==='kmz'?'application/vnd.google-earth.kmz':ext==='geojson'?'application/geo+json':ext==='csv'?'text/csv':ext==='docx'?'application/vnd.openxmlformats-officedocument.wordprocessingml.document':ext==='xlsx'?'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet':ext==='pptx'?'application/vnd.openxmlformats-officedocument.presentationml.presentation':file.type||'application/octet-stream';
...
export async function downloadEvidence(scopeId:string,id:string){if(developmentSession()){const {developmentSource}=await import('./development');const {file,bytes}=await developmentSource(scopeId,id);downloadBlob(file.name,new Blob([bytes.buffer as ArrayBuffer],{type:file.media_type}));return;}const result=await api(`files?scope=${scopeId}&download=${id}`);const u=new URL(result.url);if(u.protocol!=='https:'&&u.hostname!=='localhost')throw new Error('Invalid download address');window.location.assign(u.href);}
