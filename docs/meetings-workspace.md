...
Apply `20260909053534_meetings_workspace.sql` first, then `20260909084112_restrict_meeting_member_visibility.sql`. Both are additive and independent of earlier Operations migrations. The second migration removes direct browser-role access to the membership allowlist; the scoped RPCs remain the only product read path. Record the migration filename and SHA-256 in the production change record when applying it; do not claim an application-level migration registry or schema comment unless it has been independently verified. Earlier migrations must not be marked applied without verification. Notify PostgREST to reload its schema after application. Membership provisioning and initial source imports are administrative operations and are kept outside the public repository.
...
PostgreSQL-compatible tests cover scoped access, verified email, revoked membership, atomic import, source checksum validation, idempotency, conflicting edits, revision preservation, service-only ingestion, direct membership-table denial, RLS and invoker RPCs. Browser journeys cover discoverability, forced Development exclusion, mobile keyboard navigation, desktop/mobile notes, escaping, failed-save retry, failed-read behavior, membership clearing, explicit sign-in submission and authorization at export. Browser fixtures contain synthetic notes and mock email sending. Production role checks use a synthetic account inside a rolled-back transaction; they do not create an enduring account or bypass the user's email verification.
