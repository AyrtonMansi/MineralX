...
The run entry screen records four metrics: recovered bullion weight in grams, gold percentage of that bullion, tonnes input and processing timestamp in Queensland time (AEST). Contained gold is weight × percentage ÷ 100. Run references and creation timestamps are assigned automatically. Corrections and voids retain prior versions.
...
1. Create a dedicated Supabase project for this application after the account owner approves the provider, terms and plan. Do not attach another application's database. Choose an Australian region if available. No provider resource has been provisioned by this change.
2. Apply `supabase/migrations/202609060001_gic.sql` using the database administrator. The migration is transactional. Test it against a separate development project first.
...
4. Disable public signups in Supabase Auth. Set the site URL to the MineralX production origin and allow the exact `/gic/auth/callback` redirect. Configure password policy (minimum 12 characters), auth rate limits and production email delivery. Supabase's default mail service is restricted; test delivery before relying on resets or invitations.
...
6. For invitation links, configure the Supabase invitation email template to use `{{ .SiteURL }}/gic/auth/callback?token_hash={{ .TokenHash }}&type=invite`. For recovery use the corresponding `type=recovery`; PKCE code exchange is also supported. Never put passwords or invitation tokens into a commit or public preview.
