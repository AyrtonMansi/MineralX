...
The canonical program is the existing `mx_ops.geo_programs` entity. Home, Programs and the shared Exploration map create/edit the same UUID and version; there is no separate Home copy. Processing campaigns retain their UUIDs as compatibility projections. Program types include drilling, sampling, mapping, drone, geophysics, test work, processing, plant, maintenance, energy, civil, environmental, compliance and general work.

Navigation presents Home / Work / Geology Globe / Exploration / Pits & stockpiles / Processing / Gold / Meetings in the Development workspace. Module links choose a permitted project or facility when the current scope is incompatible; Geology Globe opens `/mineralx`, while Exploration holds the shared project registers. Home offers Create work program and Create task, with next actions, due/unassigned/blocked work, active programs and the coming fourteen days. Operational totals and introductory guidance remain available without dominating the work surface.

Program detail exposes Overview, Sequence, Records, People & equipment, Files & costs and Activity. The shared Exploration map has a typed creation menu and program selector. Actual drillholes/metres/samples require explicit linkage; spatial proximity does not complete work. `/mineralx` is Geology Globe, the explicitly labelled browser-local geology workspace. It links to shared Exploration and retains controlled migration/export; its browser-local records are not silently copied, synchronised or overwritten.
