...
export function conceptHeightForEquipment(e:Equipment){
 const name=e.name.toLowerCase(),footprint=Math.max(e.w,e.h);
...
function buildEquipmentObject(model:PlantModel,e:Equipment){
 const group=new THREE.Group();group.name=e.name;group.userData.equipmentId=e.id;
 const h=conceptHeightForEquipment(e),color=materialColor(e.group),standard=new THREE.MeshStandardMaterial({color,roughness:.72,metalness:.08}),dark=new THREE.MeshStandardMaterial({color:color.clone().multiplyScalar(.78),roughness:.8,metalness:.05}),name=e.name.toLowerCase();
 ...
 if(name.includes('tank')||name.includes('silo')||name.includes('knudsen')||name.includes('bowl')||name.includes('cyclone')){
  const radius=Math.max(.35,Math.min(e.w,e.h)*.38);primary=new THREE.Mesh(new THREE.CylinderGeometry(radius,radius,h,24),standard);
  if(name.includes('cyclone')){const cone=new THREE.Mesh(new THREE.ConeGeometry(radius*.72,Math.max(1,h*.36),24),dark.clone());cone.position.copy(sitePoint(model,[e.x+e.w/2,e.y+e.h/2],Math.max(.5,h*.12)));cone.rotation.z=Math.PI;cone.userData.equipmentId=e.id;group.add(cone);}
 }else if(name.includes('stockpile'))primary=new THREE.Mesh(new THREE.ConeGeometry(Math.max(1,Math.min(e.w,e.h)*.47),h,28),standard);
 else if(name.includes('hopper')||name.includes('bin')){primary=new THREE.Mesh(new THREE.ConeGeometry(Math.max(.5,Math.min(e.w,e.h)*.42),h,4),standard);primary.rotation.y=Math.PI/4;}
 else{primary=new THREE.Mesh(new THREE.BoxGeometry(e.w,h,e.h),standard);if(name.includes('screen'))primary.rotation.z=-.08;}
 const centre=sitePoint(model,[e.x+e.w/2,e.y+e.h/2],h/2);primary.position.copy(centre);primary.userData.equipmentId=e.id;group.add(primary);
 const base=new THREE.Mesh(new THREE.BoxGeometry(e.w+.18,.12,e.h+.18),dark);base.position.set(centre.x,.06,centre.z);base.userData.equipmentId=e.id;group.add(base);
 const edges=new THREE.LineSegments(new THREE.EdgesGeometry(primary.geometry,24),new THREE.LineBasicMaterial({color:0xffffff,transparent:true,opacity:.42}));edges.position.copy(primary.position);edges.rotation.copy(primary.rotation);edges.scale.copy(primary.scale);group.add(edges);
 return group;
}
...
export default function PlantCadSurface({model}:{model:PlantModel}){
 ...
 const focusEquipment=useCallback((id:string)=>{
  ...
  const box=new THREE.Box3().setFromObject(object),centre=box.getCenter(new THREE.Vector3()),size=box.getSize(new THREE.Vector3()),radius=Math.max(6,size.length()*1.5);controls.target.copy(centre);camera.position.set(centre.x+radius,centre.y+radius*.72,centre.z+radius);camera.updateProjectionMatrix();controls.update();
 },[]);

 useEffect(()=>{
  const container=host.current;if(!container)return;
  const scene=new THREE.Scene();scene.background=new THREE.Color('#f4f6f4');
  ...
  scene.add(new THREE.HemisphereLight(0xffffff,0x7c8b84,1.75));const sun=new THREE.DirectionalLight(0xffffff,2.1);sun.position.set(model.width*.35,Math.max(model.width,model.height)*.85,model.height*.3);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);scene.add(sun);
  ...
  for(const stream of model.streams){
   const points=stream.points.map(p=>sitePoint(model,p,DEFAULT_ROUTE_HEIGHT+(stream.kind==='service'?.35:stream.kind==='con'?.55:0))),routeMaterial=new THREE.MeshStandardMaterial({color:materialColor(stream.kind),roughness:.58,metalness:.08});
   for(let i=1;i<points.length;i++){const a=points[i-1],b=points[i],length=a.distanceTo(b);if(length<.02)continue;const radius=stream.route_type==='conveyor'?.16:.09,segment=new THREE.Mesh(new THREE.CylinderGeometry(radius,radius,1,8),routeMaterial);orientCylinder(segment,a,b);segment.castShadow=true;routes.add(segment);}
  }
 ...
 return <div className="cad-workspace">
  <div className="cad-toolbar"><div className="cad-title"><span className="ops-eyebrow">P5 · concept geometry</span><strong>Processing plant model</strong><small>{model.width} × {model.height} m yard · {model.equipment.length} equipment envelopes · {model.streams.length} process routes</small></div><div className="cad-toolbar-actions" aria-label="3D plant view controls"><button aria-pressed={projection==='perspective'} onClick={()=>setView('perspective')}>Perspective</button><button aria-pressed={projection==='top'} onClick={()=>setView('top')}>Top</button><button onClick={()=>resetView(projection)}>Fit</button><label><input type="checkbox" checked={routesVisible} onChange={e=>setRoutesVisible(e.target.checked)}/> Routes</label><label><input type="checkbox" checked={gridVisible} onChange={e=>setGridVisible(e.target.checked)}/> Grid</label></div></div>
  <div className="cad-stage-shell"><div className="cad-stage" ref={host} role="img" aria-label="Interactive three-dimensional processing plant concept model" tabIndex={0}/><aside className="cad-inspector">
   ...
   {selected?<><div className="cad-inspector-head"><span className="ops-eyebrow">{selected.id} · {groupNames[selected.group]||selected.group}</span><h2>{selected.name}</h2></div><dl><div><dt>Footprint</dt><dd>{selected.w} × {selected.h} m</dd></div><div><dt>3D height</dt><dd>{conceptHeightForEquipment(selected).toFixed(1)} m <small>concept massing</small></dd></div><div><dt>Basis</dt><dd>{selected.basis}</dd></div><div><dt>Process links</dt><dd>{connections.length} connected routes</dd></div></dl><p>{selected.note||'No engineering note recorded.'}</p><button onClick={()=>focusEquipment(selected.id)}>Focus equipment</button>{!!connections.length&&<details><summary>Connected process routes</summary>{connections.map(route=><p key={route.id}><strong>{route.id}</strong><br/><small>{route.source} → {route.target} · {route.route_type}</small></p>)}</details>}</>:<div className="cad-inspector-empty"><span className="ops-eyebrow">Model navigation</span><h2>Select equipment</h2><p>Orbit, zoom and inspect the P5 plant spatial model. Equipment colour follows process area; process routes remain source-linked to the P5 schedule.</p></div>}
