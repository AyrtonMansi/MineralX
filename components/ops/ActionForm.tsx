...
function Fields({fields,values,onChange,resources,zone,prefix='entry'}:{fields:Field[];values:any;onChange:(next:any)=>void;resources:Record<string,any[]>;zone:string;prefix?:string}){
 return <div className="ops-fields">{fields.filter(f=>!f.when||f.when(values)).map(f=>{
 ...
 if(f.type==='multi')return <fieldset className="ops-wide ops-multiselect" key={id}><legend>{f.label}</legend>{(resources[f.resource||'']||[]).map(r=><label key={r.recordId||r.id}><input type="checkbox" checked={Array.isArray(value)&&value.includes(r.recordId||r.id)} onChange={e=>set(e.target.checked?[...(Array.isArray(value)?value:[]),r.recordId||r.id]:(value||[]).filter((v:string)=>v!==(r.recordId||r.id)))}/>{r.reference||r.name||r.id} <small>{r.status||''}</small></label>)}{!(resources[f.resource||'']||[]).length&&<p>No available records. Record the upstream work first.</p>}</fieldset>;
 ...
 {f.type==='select'?<select id={id} required={f.required} value={value} onChange={e=>set(e.target.value)}><option value="">Select…</option>{f.options?.map(o=><option value={o} key={o}>{o.replaceAll('_',' ')}</option>)}{(resources[f.resource||'']||[]).map(r=><option key={r.recordId||r.id||r.user_id} value={r.recordId||r.id||r.user_id}>{r.reference||r.name||r.title||r.id}{r.net_g?' · '+r.net_g+' g':''}{r.au_percent?' · '+r.au_percent+'% Au':''}{r.status?' · '+r.status:''}</option>)}</select>:
...
export default function ActionForm({spec,initial={},recordId,expectedVersion=0,resources={},onComplete,onClose,extra={}}:{spec:ActionSpec;initial?:any;recordId?:string;expectedVersion?:number;resources?:Record<string,any[]>;onComplete?:(result:any)=>void;onClose:()=>void;extra?:any}){
 const {scope,context,send,pack,saveDraft,refresh}=useOperations(),[values,setValues]=useState<any>(()=>({...spec.defaults,...initial})),[busy,setBusy]=useState(false),[dirty,setDirty]=useState(false),[error,setError]=useState(''),[saved,setSaved]=useState(''),[review,setReview]=useState(false);const request=useRef<Command|null>(null),fingerprint=useRef('');
 const draftKey=`${spec.action}:${recordId||'new'}`;const hasPack=Boolean(pack&&pack.scope.id===scope?.id);
 ...
 <form onSubmit={async e=>{e.preventDefault();if(!scope||!context||!can||needsMfa)return;if(spec.mfa&&!review){setReview(true);return;}setBusy(true);setError('');setSaved('');try{
 ...
 <Fields fields={spec.fields} values={values} onChange={next=>{setValues(next);setDirty(true);setReview(false);}} resources={resources} zone={scope?.timezone||'Australia/Brisbane'}/>
 {review&&<section className="ops-check-answers"><h3>Check the accountable action</h3><p>{spec.title} · {scope?.name} · {recordId||'new record'} · expected revision {expectedVersion}</p><dl>{spec.fields.filter(f=>(!f.when||f.when(values))&&values[f.key]!=null).map(f=><React.Fragment key={f.key}><dt>{f.label}</dt><dd>{Array.isArray(values[f.key])?values[f.key].map((r:any)=>typeof r==='object'?Object.values(r).join(' · '):(resources[f.resource||'']||[]).find(x=>(x.recordId||x.id||x.user_id)===r)?.reference||r).join('; '):f.resource?(resources[f.resource]||[]).find(r=>(r.recordId||r.id||r.user_id)===values[f.key])?.reference||String(values[f.key]):String(values[f.key])}</dd></React.Fragment>)}</dl><button type="button" onClick={()=>setReview(false)}>Return to entry</button></section>}
 <div className="ops-form-footer"><button type="submit" className="ops-primary" disabled={busy||!can||needsMfa}>{busy?'Confirming save…':spec.mfa&&!review?'Review before '+spec.button.toLowerCase():spec.button}</button><span>{dirty?(hasPack?'Draft — device autosave pending or complete; not submitted':'Draft in this open page only — not yet submitted'):'No unsubmitted changes'}</span></div></form></section>;
}
