...
function FirstSteps({ data, scope, canWrite, onCreateProgram, onCreateTask }: { data: any; scope: { kind: string; id: string }; canWrite: boolean; onCreateProgram: () => void; onCreateTask: () => void }) {
  ...
  if (!programs.length && !tasks.length) return <section className="ops-card ops-first-steps"><p className="ops-eyebrow">START HERE</p><h2>Start with the next accountable task.</h2><p>Create a standalone task for one-off work. Create a work program only when several tasks belong to the same campaign, site project or improvement.</p><ol><li>Create the first task and assign an owner when it is ready to start.</li><li>Create a program later if the work grows into a campaign or multi-step project.</li><li>Capture the actual work in {scope.kind === 'project' ? 'Exploration or Pits & stockpiles' : 'Processing or Gold'}.</li></ol>{canWrite ? <div className="ops-actions"><button className="ops-primary" onClick={onCreateTask}>Create first task</button><button onClick={onCreateProgram}>Create work program</button></div> : <p className="ops-muted">You can review this workspace. A planning owner with write access can create the first task or a work program.</p>}</section>;
...
function JourneyStep({ number, label, title, description, href, action }: { number: string; label: string; title: string; description: string; href: string; action: string }) {
  return <Link className="ops-journey-step" href={href}>
    <span className="ops-journey-number" aria-hidden="true">{number}</span>
    <span className="ops-journey-copy"><span className="ops-eyebrow">{label}</span><strong>{title}</strong><small>{description}</small><span className="ops-journey-action">{action} <span aria-hidden="true">→</span></span></span>
  </Link>;
}
...
export default function Home() {
  ...
  const tasks = (data?.tasks || []).filter((task: any) => !['resolved', 'cancelled'].includes(task.status));
  const activePrograms = (data?.programs || []).filter((program: any) => !['completed', 'cancelled'].includes(program.data?.state || program.state));
  ...
  const canGold = scope.permissions.includes('gold.read');
  const journey = [
    scope.permissions.includes('work.read') ? <JourneyStep key="plan" number="01" label="Plan & assign" title="Set the next outcome" description="Use a task for one action, or a program when several steps belong together." href={`/ops/work?scope=${scope.id}`} action="Open work"/> : null,
    scope.kind === 'project' && canGeo ? <JourneyStep key="operate-project" number="02" label="Operate" title="Capture field evidence" description="Keep samples, holes and source files tied to the physical project record." href={`/ops/geology?scope=${scope.id}&view=samples`} action="Open Exploration"/> : scope.kind === 'facility' && canPlant ? <JourneyStep key="operate-facility" number="02" label="Operate" title="Record site work" description="Capture processing, maintenance and plant observations where they happen." href={`/ops/plant?scope=${scope.id}&view=runs`} action="Open Processing"/> : null,
    scope.permissions.includes('work.read') ? <JourneyStep key="review" number="03" label="Review & close" title="Resolve what needs attention" description="Check intelligence proposals, evidence, exceptions and reports before work is closed." href={`/ops/intelligence?scope=${scope.id}`} action="Open Intelligence"/> : <JourneyStep key="records" number="03" label="Review & close" title="Keep records current" description="Use the authorised registers and reports for this workspace." href={`/ops/reports?scope=${scope.id}`} action="Open reports"/>,
  ].filter(Boolean);
  const createProgram = () => { if (mayNavigate()) setEditor('program'); };
  const createTask = () => { if (mayNavigate()) setEditor('task'); };

  return <>
    <Heading title={scopeLabel(scope, development)} description="One place to plan, record and review this operation — with each step staying tied to the same source-backed records." action={scope.permissions.includes('work.write') && tasks.length > 0 && <button className="ops-primary" disabled={!data} onClick={createTask}>Create task</button>} />
    ...
    {data && !editor && <><section className="ops-card ops-journey" aria-labelledby="ops-journey-heading"><div className="ops-section-heading"><div><p className="ops-eyebrow">The operating path</p><h2 id="ops-journey-heading">What do you need to do?</h2><p>Follow the path from planned work to an accountable record and a clear review.</p></div></div><div className="ops-journey-grid">{journey}</div></section><FirstSteps data={data} scope={scope} canWrite={scope.permissions.includes('work.write')} onCreateProgram={createProgram} onCreateTask={createTask} /><div className="ops-home-grid"><section className="ops-card"><div className="ops-section-heading"><h2>My next actions</h2><Link href={`/ops/work?scope=${scope.id}`}>All work →</Link></div>{(myTasks.length ? myTasks : tasks.filter((task: any) => !task.responsible_id)).slice(0, 5).map((task: any) => <Link className="ops-task-line" key={task.id} href={`/ops/work?scope=${scope.id}&item=${task.id}`}><span><strong>{task.title}</strong><small>{taskDue(task, scope.timezone) || 'No due date'} · {task.responsible_id ? 'Assigned to you' : 'Unassigned team work'}</small></span><span>→</span></Link>)}{!myTasks.length && !tasks.length && <p className="ops-muted">No work is open yet. Create a task for one-off work, or a program when several tasks share one outcome.</p>}{!myTasks.length && tasks.length > 0 && <p className="ops-muted">No tasks are linked to your account. Assign planning personnel in <Link href={`/ops/work?scope=${scope.id}&view=people`}>People & workload</Link>; this never grants account access.</p>}</section><section className="ops-card"><h2>Needs attention</h2><Link className="ops-task-line" href={`/ops/work?scope=${scope.id}&filter=Due`}><strong>{due.length} due / overdue</strong><span>→</span></Link><Link className="ops-task-line" href={`/ops/work?scope=${scope.id}&filter=Blocked`}><strong>{blocked.length} blocked</strong><span>→</span></Link><Link className="ops-task-line" href={`/ops/work?scope=${scope.id}&filter=Unassigned`}><strong>{tasks.filter((task: any) => !task.responsible_id && !task.assigned_to).length} unassigned</strong><span>→</span></Link><div className="ops-actions">{scope.kind === 'project' ? canGeo && <Link href={`/ops/geology?scope=${scope.id}&view=map`}>Open Exploration map</Link> : <>{canPlant && <Link href={`/ops/plant?scope=${scope.id}`}>Processing & engineering</Link>}{canGold && <Link href={`/ops/gold?scope=${scope.id}`}>Gold next actions</Link>}</>}</div></section></div>{activePrograms.length > 0 && <><section className="ops-section-heading"><h2>Active programs</h2><Link href={`/ops/programs?scope=${scope.id}`}>All programs →</Link></section><ProgramCards data={data} scopeId={scope.id} limit={6} /></>}<section className="ops-card"><h2>Coming up</h2><p className="ops-muted">Next 14 days · {scope.timezone}. Dates stay explicitly planned until observations establish completion.</p>{tasks.filter((task: any) => { const date = taskDue(task, scope.timezone); return date && date >= today && date <= localDay(scope.timezone, new Date(Date.now() + 14 * 864e5)); }).sort((left: any, right: any) => (taskDue(left, scope.timezone) || '').localeCompare(taskDue(right, scope.timezone) || '')).slice(0, 8).map((task: any) => <Link key={task.id} className="ops-task-line" href={`/ops/work?scope=${scope.id}&item=${task.id}`}><span>{task.title}</span><span>{taskDue(task, scope.timezone)}</span></Link>)}</section></>}
    <details className="ops-card"><summary>Operational totals & source records</summary><OperationalSummary /></details>
    <details className="ops-card"><summary>How work moves through MineralX</summary><p>Create a program to plan a campaign or project; use a task for a single action. Assign people and dates, then capture actual work in Exploration or Processing. Gold records follow physical clean-ups, not forecasts. Each record has its own next action and source history.</p><p>Planning personnel do not create login accounts. Device workspace records stay on this browser; named shared work requires the commissioned backend. Download a backup before changing devices.</p></details>
  </>;
}
