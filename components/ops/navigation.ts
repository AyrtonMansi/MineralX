...
let activeEntryGuards=0;
export function entryIsOpen(){return activeEntryGuards>0;}
...
export function useEntryGuard(dirty:boolean,busy=false){useEffect(()=>{
 if(dirty||busy)activeEntryGuards++;
 ...
 return()=>{if(dirty||busy)activeEntryGuards--;window.removeEventListener('ops-before-navigate',guard);document.removeEventListener('click',links,true);window.removeEventListener('beforeunload',unload);};
 },[dirty,busy]);}
