...
function unavailable() {
  return new Response(BLANK, { status: 200, headers: { 'Content-Type': 'image/png', 'Cache-Control': 'no-store', 'X-MineralX-Tile': 'upstream-unavailable', 'X-MineralX-Tile-Status':'unavailable' } });
}
export async function GET(_request, { params }) {
  ...
  try {
    ...
    return new Response(await res.arrayBuffer(), { headers: { 'Content-Type': res.headers.get('content-type'), 'Cache-Control': 'public, max-age=86400, s-maxage=604800', 'X-MineralX-Tile-Status':'available' } });
