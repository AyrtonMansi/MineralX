...
export async function POST(request:Request){try{
 ...
 if(allowed){const db=await database();const {error}=await db.auth.signInWithOtp({email:email.toLowerCase(),options:{shouldCreateUser:false,emailRedirectTo:company.url+'/gic/auth/callback'}});if(error)throw new OpsError('unavailable','The identity service could not send the sign-in email. Wait a minute and retry, or use your existing password.');
