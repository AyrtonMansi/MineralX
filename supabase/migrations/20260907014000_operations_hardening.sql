-- Forward-only hardening of the published Operations candidate. No legacy tables are rewritten.
begin;
...
begin
 ...
 if p_action in('weight.record','assay.stage') then
  ...
  if p_action='weight.record' then
   ...
   perform mx_ops.rule(substring(trim(p->>'raw_result') from '(%|g/kg|g/t)$') is null or substring(trim(p->>'raw_result') from '(%|g/kg|g/t)$')=p->>'unit','Reported text and selected units disagree');
...
create or replace function mx_ops.core_command(p_scope uuid,p_action text,p_id uuid,p_expected integer,p jsonb) returns jsonb language plpgsql security definer set search_path='' as $$
declare w mx_ops.work_items;f mx_ops.files;d mx_ops.documents;begin
 if p_action in('work.save','work.resolve') then
  ...
  perform mx_ops.rule(nullif(p->>'supersedes','') is null or exists(select 1 from mx_ops.documents previous where previous.id=(p->>'supersedes')::uuid and previous.scope_id=p_scope),'A document may supersede only a revision in the same workspace');
