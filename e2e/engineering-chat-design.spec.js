...
test('ChatGPT draft geometry opens directly in the real 3D Engineering surface without a database write',async({page})=>{
 const protectedCalls=[];page.on('request',request=>{if(new URL(request.url()).pathname==='/api/ops/engineering-design')protectedCalls.push(request.url());});
 const draft=JSON.stringify([{type:'move_equipment',equipmentId:'ROM',x:9,y:41}]);
 const query=new URLSearchParams({scope:facility,view:'engineering',surface:'cad',mode:'development',draft});
 await page.goto(`/ops/plant?${query.toString()}`);
