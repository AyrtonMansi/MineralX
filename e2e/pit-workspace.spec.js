...
test('failed device saves retain work, retry commits, and stale versions cannot overwrite newer scenarios',async({page})=>{
 ...
 await page.goto('/ops/pit?scope=de000000-0000-4000-8000-000000000003&mode=development');await expect(page.getByRole('heading',{name:'Pits & stockpiles',exact:true})).toBeVisible();await expect(page.getByRole('combobox',{name:'Saved on this device',exact:true})).toHaveCount(0);
});
