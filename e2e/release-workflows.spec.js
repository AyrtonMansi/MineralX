...
test('human field-to-result workflow survives reload and exports a complete backup',async({page})=>{
 ...
 await page.getByRole('checkbox',{name:'Select BAG-1 for dispatch'}).check();await page.getByLabel('Receiving laboratory',{exact:true}).fill('Fixture laboratory');await page.getByRole('button',{name:'Prepare manifest (1)',exact:true}).click();await saved(page);
 await nav(page,'Review');await page.getByText('DSP-0001 · Fixture laboratory · prepared',{exact:true}).click();await page.getByLabel('Shipment / tracking reference',{exact:true}).fill('TRACK-1');await page.getByLabel('Actual shipping date',{exact:true}).fill('2026-09-06');await page.getByRole('button',{name:'Record shipment',exact:true}).click();await saved(page);await page.getByRole('checkbox',{name:'BAG-1',exact:true}).check();await page.getByLabel('Receipt reference',{exact:true}).fill('RECEIPT-1');await page.getByRole('button',{name:'Record laboratory receipt',exact:true}).click();await saved(page);
...
test('drill capture rejects overlap and preserves physical bag identity',async({page})=>{
 ...
 for(const [label,value] of [['Hole ID','DH-1'],['Actual drilled depth (m)','20'],['Latitude (WGS84)','-21'],['Longitude (WGS84)','145'],['Azimuth (degrees)','90'],['Dip (degrees)','-60']])await page.getByLabel(label,{exact:true}).fill(value);
...
// Delivery-boundary regressions: exercise the actual forms and IndexedDB, not a mocked store.
test('reference controls need no location and normalized IDs fail visibly without a runtime error',async({page})=>{
 const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.goto('/mineralx');await createProject(page,'Control fixture');
 await page.getByRole('button',{name:'+ Collect sample',exact:true}).click();await page.getByLabel('Bag / sample ID',{exact:true}).fill('QC-1');await page.getByRole('combobox',{name:'QA/QC type',exact:true}).selectOption('blank');
 await expect(page.getByLabel('Latitude (WGS84)',{exact:true})).toHaveCount(0);await page.getByRole('button',{name:'Save record',exact:true}).click();await saved(page);await expect(page.getByRole('region',{name:'Samples workspace'})).toBeVisible();
 const p=(await db(page)).data.projects[0];expect(p.samples[0].lat).toBeNull();expect(p.samples[0].lng).toBeNull();expect(p.samples[0].qaqcType).toBe('blank');
 await page.getByRole('button',{name:'+ Collect sample',exact:true}).click();await page.getByLabel('Bag / sample ID',{exact:true}).fill('qc-1');await page.getByRole('combobox',{name:'QA/QC type',exact:true}).selectOption('blank');await page.getByRole('button',{name:'Save record',exact:true}).click();await expect(page.locator('.mxf-panel [role=alert]')).toContainText('already exists');expect((await db(page)).data.projects[0].samples).toHaveLength(1);expect(errors).toEqual([]);
});
test('partial custody blocks review, completion releases, and repeated certificate imports stay idempotent',async({page})=>{
 await page.goto('/mineralx');await createProject(page,'Custody fixture');await collect(page,'ONE');await collect(page,'TWO');
 for(const id of ['ONE','TWO'])await page.getByRole('checkbox',{name:`Select ${id} for dispatch`,exact:true}).check();await page.getByLabel('Receiving laboratory',{exact:true}).fill('Test lab');await page.getByRole('button',{name:'Prepare manifest (2)',exact:true}).click();await saved(page);
 let p=(await db(page)).data.projects[0];expect(p.dispatches[0].status).toBe('prepared');expect(p.dispatches[0].dispatchedAt).toBeUndefined();
 await nav(page,'Review');await page.getByText('DSP-0001 · Test lab · prepared',{exact:true}).click();await page.getByLabel('Shipment / tracking reference',{exact:true}).fill('ACTUAL-SHIPMENT');await page.getByRole('button',{name:'Record shipment',exact:true}).click();await saved(page);
 await page.getByRole('checkbox',{name:'ONE',exact:true}).check();await page.getByLabel('Receipt reference',{exact:true}).fill('PARTIAL');await page.getByRole('button',{name:'Record laboratory receipt',exact:true}).click();await saved(page);
 await page.getByLabel('Certificate / batch reference',{exact:true}).fill('REAL-CERT');await page.getByLabel('CSV result text',{exact:true}).fill('sample_id,Au_gpt\none,<5');await page.getByRole('button',{name:'Stage results for review',exact:true}).click();await saved(page);await page.getByLabel('Reviewing geologist',{exact:true}).fill('Local reviewer');await expect(page.getByRole('button',{name:'Release reviewed results',exact:true})).toBeDisabled();
 await page.getByRole('checkbox',{name:'TWO',exact:true}).check();await page.getByLabel('Receipt reference',{exact:true}).fill('COMPLETE');await page.getByRole('button',{name:'Record laboratory receipt',exact:true}).click();await saved(page);await page.getByRole('button',{name:'Release reviewed results',exact:true}).click();await saved(page);
 await page.getByRole('button',{name:'Stage results for review',exact:true}).click();await saved(page);p=(await db(page)).data.projects[0];expect(p.assayBatches).toHaveLength(1);expect(p.samples[0].assayHistory).toHaveLength(1);expect(p.samples[0].detectionLimits.Au).toBe(5);expect(p.samples[0].assays.Au).toBeUndefined();
 await nav(page,'Samples');await page.locator('.mxf-sample>summary').filter({hasText:'ONE'}).click();page.once('dialog',d=>d.accept('Duplicate field location retained for audit'));await page.getByRole('button',{name:'Archive sample',exact:true}).click();await saved(page);expect((await db(page)).data.projects[0].samples[0].archivedAt).toBeTruthy();await page.getByRole('checkbox',{name:'Include archived records',exact:true}).check();await expect(page.locator('.mxf-sample>summary').filter({hasText:'ONE'})).toContainText('archived');
});
test('an aborted storage transaction retains the open capture draft and reports failure',async({page})=>{
 await page.goto('/mineralx');await createProject(page,'Storage fixture');
 await page.evaluate(()=>{const original=IDBObjectStore.prototype.put;IDBObjectStore.prototype.put=function(...args){const result=original.apply(this,args);if(this.name==='records'&&args[1]==='workspace'&&args[0]?.data?.projects?.some(p=>p.samples.length))this.transaction.abort();return result;};});
 await page.getByRole('button',{name:'+ Collect sample',exact:true}).click();await page.getByLabel('Bag / sample ID',{exact:true}).fill('UNSAVED');await page.getByLabel('Latitude (WGS84)',{exact:true}).fill('-21');await page.getByLabel('Longitude (WGS84)',{exact:true}).fill('145');await page.getByRole('textbox',{name:'Field notes',exact:true}).fill('These notes must survive failed persistence');await page.getByRole('button',{name:'Save record',exact:true}).click();
 await expect(page.locator('.mxf-status')).toHaveAttribute('data-state','blocked');await expect(page.getByLabel('Bag / sample ID',{exact:true})).toHaveValue('UNSAVED');expect((await db(page)).data.projects[0].samples).toHaveLength(0);expect(await page.evaluate(()=>Object.entries(localStorage).filter(([k])=>k.startsWith('mx-field-draft-')).some(([,v])=>v.includes('These notes must survive')))).toBe(true);
});
